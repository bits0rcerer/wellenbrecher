...
use nftables::expr::{Expression, NamedExpression, Payload, CT};
...
impl ConnectionLimit {
    pub fn new(port: u16, connections_per_ip: u32) -> Self {
    ...
    pub fn apply(&self) -> Result<(), nftables::helper::NftablesError> {
        ...
        let add_limit_rule = |batch: &mut Batch, table: &Table, chain: &Chain, proto: &str| {
            batch.add_cmd(NfCmd::Add(NfListObject::Rule(Rule {
                ...
                expr: vec![
                    ...
                    Statement::Meter(Meter {
                        ...
                        key: Expression::Named(NamedExpression::Payload(Payload {
        ...
        add_limit_rule(&mut batch, &self.table_ipv4, &self.in_chain_ipv4, "ip");
        add_limit_rule(&mut batch, &self.table_ipv6, &self.in_chain_ipv6, "ip6");
